function AAM = build_model_2d(shape_data, app_data, varargin)
% AAM = build_model_2d(shape_data, app_data)
% AAM = build_model_2d(shape_data, app_data, 'triangulation', triangulation)
%
...
%                         PARAMETERS
%
% shape_data A Nx2xS matrix containing S shapes and N landmarks,
%            that is 2D points stored on rows, in image coordinates: [i j]
%
% app_data A HxWxCxS array of 2D images, with pixels arranged such
%          that app_data(i,j,c,k) actually returns the pixel value
...
% Author: Luca Vezzaro (elvezzaro@gmail.com)
	...
	if isa(app_data, 'float') == 0
		warning 'Appearance data is not in floating point format, assuming unsigned integer data.'
	...
	%% DEBUG STUFF
	%%
	%for i=1:size(shape_data, 3)
	%	imshow(app_data(:,:,:,i));
	%	hold on;
	%	triplot(shape_triangles, shape_data(:,2,i), shape_data(:,1,i));
	%	pause;
	%	hold off;
	%end
	%return;
    ...
    hold on;
    for i = 1:size(shape_data,3)
        triplot(shape_triangles, aligned_data(:,2,i), aligned_data(:,1,i));
    end
    hold off;
...
%	return;
	...
	app_matrix = zeros(modelw*modelh*size(app_data,3), ns);
	
	% For each image
	for i=1:ns    
		app_matrix(:,i) = reshape(pa_warp(AAM, shape_data(:,:,i), app_data(:,:,:,i)), [(modelw*modelh*size(app_data,3)) 1]);
	end
...
%	fprintf('Image %d\n.', i);
%	
%	imshow(reshape(app_matrix(:,i), [modelh modelw size(app_data,3)]));
%	
%	hold on;
%	
...
%return;
	...
	[pc eiv] = app_pca(app_matrix, 0.98);
	clear app_matrix;
